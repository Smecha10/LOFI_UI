...
function hourGrade(h) {
  const table = [
    { h:0,  tint:'#161328', aa:0.55, warmth: 0.0,  bright: .70 },
    { h:5,  tint:'#2c1d42', aa:0.45, warmth: 0.1,  bright: .80 },
    { h:7,  tint:'#8ccabb', aa:0.35, warmth: 0.3,  bright: 1.05},
    { h:12, tint:'#ffebb5', aa:0.18, warmth: 0.6,  bright: 1.15},
    { h:17, tint:'#ff8c61', aa:0.35, warmth: 0.90, bright: 1.02},
    { h:19, tint:'#542a59', aa:0.45, warmth: 0.5,  bright: .85 },
    { h:21, tint:'#221438', aa:0.55, warmth: 0.25, bright: .75 },
  ];
...
function PixelRoom({ hour, lampOn, catAlert, activity, now, logLines, piStats }) {
  ...
  useEffect(() => {
    ...
    const P = {
      wall:       isNight ? '#2a1a36' : isDusk ? '#8a405a' : '#c27c62',
      wallD:      isNight ? '#1b1028' : isDusk ? '#602a42' : '#8c4c3e',
      wallH:      isNight ? '#402a50' : isDusk ? '#aa5872' : '#d89b82',
      wallTrim:   isNight ? '#100a1c' : '#2e1410',
      floor:      isNight ? '#2b1b22' : '#6a4034',
      floorD:     isNight ? '#1b0e14' : '#4a241b',
      floorH:     isNight ? '#402632' : '#8c584a',
      sky1:       isNight ? '#0c0d1c' : isDusk ? '#d4654b' : '#8ac2eb',
      sky2:       isNight ? '#1a142c' : isDusk ? '#753468' : '#e4c492',
      sky3:       isNight ? '#35203A' : isDusk ? '#4a1b4d' : '#ffe0b0',
      star:       '#fff4c8',
      city:       isNight ? '#120a22' : '#2c1e2e',
      cityD:      isNight ? '#080416' : '#1e1422',
      cityLit:    '#ffd070',
      cityLitB:   '#aaddff',
      frame:      '#100a06',
      desk:       '#5c341e',
    ...
    for (let r = 0; r < rows; r++){
      ...
      if (itm.type === 'books'){
        ...
        drawCat(ctx, 357, 105, P, catAlert, now);
    ...
    const ckX = 235, ckY = 60, ckW = 82, ckH = 86;
    ...
    // ===== monitor on LEFT side of desk (User's choice) =====
    const monX = 79, monY = 116, monW = 78, monH = 48;
    ...
    // ===== Raspberry Pi with OLED =====
    const piX = 18, piY = 135, piW = 57, piH = 40;
    ...
    // ===== keyboard =====
    const kbX = 82, kbY = 168, kbW = 72, kbH = 8;
    ...
    // ===== lamp =====
    const lampX = 200, lampY = 142;
    rect(lampX + 5, lampY + 30 - 2, 14, 2, P.frame);
    rect(lampX + 7, lampY + 30 - 4, 10, 2, P.frame);
    rect(lampX + 10, lampY + 30 - 22, 4, 18, P.frame);
    ...
    rect(lampX + 8, lampY + 30 - 3, 2, 1, lampOn ? '#7cd992' : '#ff6b6b');
    ...
    // ===== Terrence =====
    drawTerrence(ctx, 141, 113 + 76, P, activity, now);

    // ===== floor plants =====
    drawBigPlant(ctx, 274, 172, P, now);
    drawPlant(ctx, 450, 188, P, 1.0);
...
function Scene({ tweaks, activeSkillIds, onOpenSkill, onOpenRoute, now }){
  ...
  const d = new Date(now || animNow);
  const h = tweaks.hourOverride >= 0 ? tweaks.hourOverride : (d.getHours() || 0);
